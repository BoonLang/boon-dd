...
use boon_dd::{BoonTime, Diff, EncodedTime, MonitorRecord, NodeId, OwnerKey, RenderCommand, SmokeOutput, SourceAction};
...
impl GeneratedGraphHandles {
    pub fn submit_text_and_drain(
        ...
        while self.probe.less_than(&target) {
            if steps == max_steps {
                return Err(format!("generated graph probe stalled at {target} after {steps} steps"));
            }
            worker.step();
            steps += 1;
        }
...
pub fn build_dataflow(
    ...
    worker.dataflow::<EncodedTime, _, _>(|scope| {
        ...
        let rendered = events.map(|_| ()).count().map(|(_key, count)| (0 + count as i64).to_string());
        rendered
            .inspect(move |(text, time, diff)| {
                if *diff > 0 {
                    output_in_graph.lock().expect("generated output lock poisoned").push(SmokeOutput {
