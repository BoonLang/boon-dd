...
pub fn build_dataflow(worker: &mut timely::worker::Worker) -> GeneratedGraphHandles {
    ...
    worker.dataflow::<EncodedTime, _, _>(|scope| {
        ...
        let rendered_values = (events
            ...
            .map(|(_sequence, event)| generated_bound_source_value(&event)))
        .flat_map(|matched_value| {
            let matched_value = matched_value.clone();
            if boon_dd::dd_pattern_matches(&matched_value, "Active") {
                vec![DdValue::Text("visible".to_owned())]
            } else if boon_dd::dd_pattern_matches(&matched_value, "Completed") {
                Vec::<DdValue>::new()
            } else {
                Vec::<DdValue>::new()
            }
