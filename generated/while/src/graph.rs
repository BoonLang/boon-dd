...
fn source_id_for_path(path: &str) -> String {
    match path {
        "selected_filter" => "SelectedFilter".to_owned(),
...
pub fn build_dataflow(worker: &mut timely::worker::Worker) -> GeneratedGraphHandles {
    ...
    worker.dataflow::<EncodedTime, _, _>(|scope| {
        ...
        let rendered = (render_events.clone().flat_map(|(_sequence, event)| {
            generated_event_identity(&event)
                .map(|identity| (identity, generated_source_text(&event)))
        }))
        .flat_map(|(identity, matched_text)| {
            let values = {
                if matched_text == "Active" {
                    vec!["visible".to_owned()]
                } else if matched_text == "Completed" {
                    Vec::<String>::new()
                } else {
                    Vec::<String>::new()
                }
            ...
            values
                .into_iter()
                .map(move |value| (identity.clone(), value))
        })
