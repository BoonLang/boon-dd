...
pub fn checked_scenario_steps() -> Vec<boon_dd::ScenarioStep> {
    serde_json::from_str("[{\"description\":\"remove retained row\",\"events\":[{\"Source\":{\"source\":\"item.sources.remove_button.event.press\",\"owner\":\"item-1\",\"generation\":0,\"value\":\"EmptyRecord\"}}],\"actions\":[{\"source\":\"item.sources.remove_button.event.press\",\"owner\":\"item-1\",\"generation\":0,\"value\":\"EmptyRecord\"}],\"commands\":[],\"expect_text\":\"removed\",\"expect_monitor_changed\":[\"DocumentOutput\",\"DocumentText\"]}]").expect("checked scenario steps should deserialize")
}
