...
pub fn checked_scenario_steps() -> Vec<boon_dd::ScenarioStep> {
    serde_json::from_str("[{\"description\":\"create record\",\"events\":[{\"Source\":{\"source\":\"store.sources.save_button.event.press\",\"owner\":null,\"generation\":null,\"value\":\"EmptyRecord\"}}],\"actions\":[{\"source\":\"store.sources.save_button.event.press\",\"owner\":null,\"generation\":null,\"value\":\"EmptyRecord\"}],\"commands\":[],\"expect_text\":\"saved\",\"expect_monitor_changed\":[\"DocumentOutput\",\"DocumentText\"]}]").expect("checked scenario steps should deserialize")
}
