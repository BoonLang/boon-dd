...
pub fn build_dataflow(worker: &mut timely::worker::Worker) -> GeneratedGraphHandles {
    ...
    worker.dataflow::<EncodedTime, _, _>(|scope| {
        ...
        let rendered_values = ((((events.clone().map(|_| {
            DdValue::List(vec![
                DdValue::Record(vec![
                    ("title".to_owned(), DdValue::Text("milk".to_owned())),
                    ("completed".to_owned(), DdValue::Tag("False".to_owned())),
                ]),
                DdValue::Record(vec![
                    ("title".to_owned(), DdValue::Text("bread".to_owned())),
                    ("completed".to_owned(), DdValue::Tag("False".to_owned())),
                ]),
                DdValue::Record(vec![
                    ("title".to_owned(), DdValue::Text("done".to_owned())),
                    ("completed".to_owned(), DdValue::Tag("True".to_owned())),
                ]),
            ])
        }))
        .map(|pipe_input| match pipe_input {
            DdValue::List(values) => DdValue::List(
                values
                    .into_iter()
                    .filter(|item_value| {
        ...
        .map(|pipe_input| match pipe_input {
            DdValue::List(values) => DdValue::Number(values.len() as i64),
            _ => DdValue::Number(0),
        }))
