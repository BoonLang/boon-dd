...
use boon_dd::{
    BoonNumber, BoonTime, Diff, EncodedTime, GeneratedEventIdentity, GeneratedSourceEvent,
    GeneratedSourceEventPayload, MonitorRecord, NodeId, OwnerKey, RenderCommand, SmokeOutput,
...
fn source_id_for_path(path: &str) -> String {
    match path {
        other => panic!("unknown generated source action path `{other}`"),
    }
}
...
pub fn build_dataflow(worker: &mut timely::worker::Worker) -> GeneratedGraphHandles {
    ...
    worker.dataflow::<EncodedTime, _, _>(|scope| {
        ...
        let rendered = ((((render_events.clone().flat_map(|(_sequence, event)| {
            generated_event_identity(&event).map(|identity| {
                (
                    identity,
                    vec![
                        {
                            let mut record = std::collections::BTreeMap::<String, String>::new();
                            record.insert("title".to_owned(), "milk".to_owned());
                            record.insert(
                                "completed".to_owned(),
                                if false {
                                    "True".to_owned()
                        ...
                        {
                            let mut record = std::collections::BTreeMap::<String, String>::new();
                            record.insert("title".to_owned(), "bread".to_owned());
                            record.insert(
                                "completed".to_owned(),
                                if false {
                                    "True".to_owned()
                        ...
                        {
                            let mut record = std::collections::BTreeMap::<String, String>::new();
                            record.insert("title".to_owned(), "done".to_owned());
                            record.insert(
                                "completed".to_owned(),
                                if true {
                                    "True".to_owned()
        ...
        .map(|(identity, pipe_input)| {
            (identity, {
                let mut retained = Vec::new();
                for item_value in pipe_input {
                    if !({
                        ...
                        retained.push(item_value.clone());
                    }
                }
                retained
            })
        ...
        .map(|(identity, pipe_input)| {
            (identity, {
                let values = pipe_input;
                generated_list_len(&values)
            })
        }))
        .map(|(identity, pipe_input)| (identity, (pipe_input).to_string())))
        .map(|(identity, pipe_input)| (identity, pipe_input))
        .map(|(identity, text)| (identity.owner_key, text));
        rendered
