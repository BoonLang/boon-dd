...
pub fn checked_scenario_steps() -> Vec<boon_dd::ScenarioStep> {
    serde_json::from_str("[{\"description\":\"row state update\",\"events\":[],\"actions\":[],\"commands\":[],\"expect_text\":\"2 active\",\"expect_monitor_changed\":[\"DocumentOutput\",\"DocumentText\"]}]").expect("checked scenario steps should deserialize")
}
