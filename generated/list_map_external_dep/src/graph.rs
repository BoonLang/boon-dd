...
pub fn build_dataflow(worker: &mut timely::worker::Worker) -> GeneratedGraphHandles {
    ...
    worker.dataflow::<EncodedTime, _, _>(|scope| {
        ...
        let rendered_values = (((events.clone().map(|_| {
            DdValue::List(vec![
                DdValue::Text("a".to_owned()),
                DdValue::Text("b".to_owned()),
                DdValue::Text("c".to_owned()),
            ])
        }))
        .map(|pipe_input| match pipe_input {
            DdValue::List(values) => DdValue::List(
                values
                    .into_iter()
                    .map(|item_value| DdValue::Text((item_value.clone()).text().to_uppercase()))
                    .collect(),
            ),
            other => other,
        }))
        .map(|pipe_input| match pipe_input {
            DdValue::List(values) => DdValue::Text(
                values
                    .into_iter()
                    .map(DdValue::text)
                    .collect::<Vec<_>>()
                    .join(&(DdValue::Text(",".to_owned())).text()),
            ),
