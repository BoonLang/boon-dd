...
pub fn checked_scenario_steps() -> Vec<boon_dd::ScenarioStep> {
    serde_json::from_str("[{\"description\":\"map list with external dependency\",\"events\":[],\"actions\":[],\"commands\":[],\"expect_text\":\"A,B,C\",\"expect_monitor_changed\":[\"DocumentOutput\",\"DocumentText\"]}]").expect("checked scenario steps should deserialize")
}
