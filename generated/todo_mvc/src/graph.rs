...
fn source_id_for_path(path: &str) -> String {
    match path {
        "store.sources.new_todo_input.event.key_down.key" => {
            "StoreSourcesNewTodoInputEventKeyDownKey".to_owned()
        }
        "store.sources.new_todo_input.text" => "StoreSourcesNewTodoInputText".to_owned(),
...
pub fn build_dataflow(worker: &mut timely::worker::Worker) -> GeneratedGraphHandles {
    ...
    worker.dataflow::<EncodedTime, _, _>(|scope| {
        ...
        let rendered = (render_events.clone().flat_map(|(_sequence, event)| {
            generated_event_identity(&event).map(|identity| (identity, "2 todos".to_owned()))
        }))
