...
pub fn build_dataflow(worker: &mut timely::worker::Worker) -> GeneratedGraphHandles {
    ...
    worker.dataflow::<EncodedTime, _, _>(|scope| {
        ...
        let rendered = events
            ...
            .map(|_| {
                (match match GeneratedValue::List(vec![GeneratedValue::Text("milk".to_owned())]) {
                    GeneratedValue::List(mut values) => {
                        values.push(GeneratedValue::Text("bread".to_owned()));
                        GeneratedValue::List(values)
                    }
                    other => other,
                } {
                    GeneratedValue::List(values) => GeneratedValue::Text(
                        values
                            .into_iter()
                            .map(GeneratedValue::text)
                            .collect::<Vec<_>>()
                            .join(&(GeneratedValue::Text(",".to_owned())).text()),
                    ),
