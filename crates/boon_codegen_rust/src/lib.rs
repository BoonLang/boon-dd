...
fn generated_bound_source_owner(event: &GeneratedSourceEvent) -> OwnerKey {
    match event {
        ...
        GeneratedSourceEvent::Dynamic { family_id, owner_key, generation, payload: _ }
            if generated_dynamic_source_id_is_bound(family_id, owner_key, *generation) => owner_key.clone(),
...
impl RenderCollectionCode {
    fn into_text_collection(self) -> String {
        match self.kind {
            ...
            RenderKind::Number => format!("({}).map(|value| value.to_string())", self.code),
            RenderKind::Bool => format!(
                "({}).map(|value| if value {{ \"True\".to_owned() }} else {{ \"False\".to_owned() }})",
                self.code
            ),
...
fn stage_collection_graph_code(
    ...
    match &stage.operation {
        ...
        boon_dd::DdRenderGraphOperation::Then { body } => {
            ...
            RenderCollectionCode {
                kind: value.kind,
                code: format!("({}).map(|_| {})", input.code, value.code),
            }
        }
        boon_dd::DdRenderGraphOperation::Hold { body, binder } => {
            hold_collection_graph_code(graph, input, body, binder, env)
...
fn call_graph_collection_code(
    ...
    match callee {
        "Math/sum" => RenderCollectionCode {
            kind: RenderKind::Number,
            code: format!(
                "({}).map(|_event| ()).count().map(|(_key, count)| count as i64)",
                input.code
            ),
        },
        ...
        | "Geometry/intersects" => {
            ...
            RenderCollectionCode {
                kind: value.kind,
                code: format!("({}).map(|pipe_input| {})", input.code, value.code),
            }
        }
...
fn hold_collection_graph_code(
    ...
    RenderCollectionCode {
        kind: next.kind,
        code: format!(
            "({}).map(|_event| ()).count().map(move |(_key, count)| {})",
...
mod tests {
    ...
    fn collection_matches_lower_skip_as_empty_output() {
        ...
        let module = generated_graph_module(&plan);
        assert!(module.contains(".flat_map(|matched_text|"));
