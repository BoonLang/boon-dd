use boon_dd::{
    ...
    ScenarioStep, SmokeOutput, SourceAction,
};
use std::collections::BTreeMap;

const REMOVED_RUNTIME_HOST_MESSAGE: &str = "boon_runtime_host no longer executes Boon semantics; use a checked generated Timely/Differential graph runtime";

pub fn run_dd_graph_scenario(
    _source_path: impl Into<String>,
    _source_text: impl Into<String>,
    _scenario_text: &str,
) -> Result<SmokeOutput, String> {
    Err(REMOVED_RUNTIME_HOST_MESSAGE.to_owned())
}
...
impl ThreadedGraphSession {
    ...
    pub fn submit_host_tick_and_drain(&self, _epoch: u64) -> Result<SmokeOutput, String> {
        Err(REMOVED_RUNTIME_HOST_MESSAGE.to_owned())
    }

    pub fn submit_action_and_drain(
        &self,
        _action: SourceAction,
        _epoch: u64,
    ) -> Result<SmokeOutput, String> {
        Err(REMOVED_RUNTIME_HOST_MESSAGE.to_owned())
    }

    pub fn retract_last_and_drain(&self, _epoch: u64) -> Result<SmokeOutput, String> {
        Err(REMOVED_RUNTIME_HOST_MESSAGE.to_owned())
    }
}
